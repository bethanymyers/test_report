...
author: Principal Investigator - Bethany Myers, `r params$institution`
...
__Do Reputable Open Access Journals Require Open Data Sharing?__
...
[^1]:in particular, the choice to include open access journals merely because of their use of the Open Journal Systems (OJS) hosting platform; and the choice to exclude non_English language journals.
...
```{r plot_2}
ggplot(data=doaj_seal, aes(doaj_seal$JnlLicense, fill=doaj_seal$JnlLicense)) + stat_count() + labs(x="License", y="Count", title = "DOAJ Seal by Country") + guides(fill=guide_legend("License"))
...
ggplot(data=selected, aes(selected$JnlLicense, fill=selected$JnlLicense)) + stat_count() + labs(x="License", y="Count", title = titlev) + guides(fill=guide_legend("License"))
...
__Bethany Myers, Principal Investigator, `r params$institution`__

## Researcher ID

__ORCID: your ORCID number__
...
The complete data set is available in searchable and  broweseable format as [Annex 10.1](#annex-table) at the end of this document. 
...
Investigators will examine the websites of each journal listed in the `doaj_seal_enhanced.csv` file to determine whether the data sharing policy is included in the Instructions to Authors. _The Coding Framework_ published by @Castro_2017 will be applied.
...
[^2]:The Legal Interop Guidelines are posted at https://www.rd-alliance.org/rda-codata-legal-interoperability-research-data-principles-and-implementation-guidelines-now.
